...
export class TicketmasterHttpService {
  constructor(
    ...
    private readonly ticketmasterCredentialsConfig: ConfigType<
      typeof TicketmasterCredentialsConfig
    >,
    protected readonly httpService: HttpService,
  ) {}

  private async _request<TReponse>(
  ...
  async eventSearch(
    ...
    AxiosResponse<TicketMasterPagination<{ events: TicketMasterEvent[] }>>
  > {
    return this._request<
      TicketMasterPagination<{ events: TicketMasterEvent[] }>
    >({
      url: `events.json?${query}`,
      params: {
        size: 200,
      },
      method: 'GET',
